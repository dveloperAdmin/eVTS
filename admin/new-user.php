                      ...
                      <div class="col-md-6">
                        ...
                        <div class="card">
                          ...
                          <form action="" method="post">
                            ...
                            <?php if (mysqli_num_rows($sql_branch_num) >0 && (in_array($user_role, array("Developer", "Super Admin")))) { ?>
                                  ...
                                  <select name="user_role" id="branch" required class="form-control">
                                    ...
                                    <?php
                                         while ($branch_detils = mysqli_fetch_assoc($sql_branch_num)) { ?>
                                            <option value="<?= $branch_detils['branch_code'] ?>"><?= $branch_detils['branch_name']; ?> </option>
                                    <?php } ?>
                                  </select>
                                </div>
                              </div>
                            ...
                            <?php } else{?>
                              <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                  ...
                                  <?php
                                      ...
                                      $sql_emp_data = mysqli_query($conn, "select eomploye_details.* from eomploye_details join user on eomploye_details.EmployeeId = user.EmployeeId where eomploye_details.`BranchId`='$branch_id'");
      
                                      while ($emp_data = mysqli_fetch_assoc($sql_emp_data)) {
                                    ?>
                                    <option data-value="<?= $emp_data['EmployeeId'] ?>"><?= $emp_data['EmployeeName']; ?>
                                    </option>
                                    <?php } ?>
                                  </datalist>
                                  <input type="hidden" name="user_name" id="emp-hidden">
                              ...
                              <div class="user-entry">
                                <button type="reset" class="btn waves-effect waves-light btn-inverse btn-outline-inverse"><i
                                    class="icofont icofont-exchange"></i>Cancel</button>
                                <button type="submit" 
                                class="btn waves-effect waves-light btn-primary btn-outline-primary"
                                  name="u_submit"><i class="fa fa-user-plus"
                                    style="font-size: 20px;margin-right: 10px;"></i>Add User</button>
                              </div>
                          </from>
                          </div>
                        </div>
